...
//
// Melhorias aplicadas:
//   1. fastWrite() — atualização dos canais A/B em uma única transação I2C
...
static const size_t   FRAME_SIZE    = 12;
...
static void setIdleOutput() {
  // Mantém os 4 canais em midscale quando o sistema está em idle
...
static bool tryReadOneFrame(Frame &out) {
  ...
  while (Serial.available() >= 1) {
    ...
    if (buf[0] != MAGIC0 || buf[11] != MAGIC1) continue;

    uint8_t chk = checksum8(buf, 10);
    if (chk != buf[10]) { g_checksumErrors++; continue; }
    ...
    out.dac_v = clamp12((uint16_t)buf[5] | ((uint16_t)buf[6] << 8));
    out.dac_i = clamp12((uint16_t)buf[7] | ((uint16_t)buf[8] << 8));
    out.flags = buf[9];
...
static inline void outputFrame(const Frame &fr) {
  // Estratégia 1: atualiza V/I e mantém os demais canais em idle
  dac.fastWrite(fr.dac_v, fr.dac_i, DAC_IDLE_CODE, DAC_IDLE_CODE);
  g_playedFrames++;
}
