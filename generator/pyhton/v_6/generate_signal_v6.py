...
MAX_OUT_WAITING = 384          # ~32 frames × 12 bytes
...
def build_samples(cfg: ComtradeCfg, bdat_path: Path) -> Tuple[List[SamplePacket], float, float, float, float]:
    ...
    return samples, v_nom, i_nom, v_clip, i_clip


def export_receiver_config_json(
    *,
    out_path: Path,
    cfg_path: Path,
    bdat_path: Path,
    ...
    # Inverso do mapeamento do gerador:
    # eng = ((v_out - V_MID) / V_AMP) * clip_peak
    # => escala sugerida no receptor (eng por volt AC medido):
    # scale = clip_peak / V_AMP
    payload = {
        "schema_version": 1,
        ...
        "comtrade_reference": {
            "v_nom_peak": v_nom,
            "i_nom_peak": i_nom,
            "v_nom_rms": v_nom / math.sqrt(2.0),
            "i_nom_rms": i_nom / math.sqrt(2.0),
            "v_clip_peak": v_clip,
            "i_clip_peak": i_clip,
        ...
        "receiver_recommendation": {
            ...
            "offset_v": V_MID,
            "offset_i": V_MID,
            "v_scale_eng_per_volt": (v_clip / V_AMP) if V_AMP > 0 else 1.0,
            "i_scale_eng_per_volt": (i_clip / V_AMP) if V_AMP > 0 else 1.0,
            "freq_hz": cfg.freq_hz,
            "sample_rate_hz": cfg.sample_rate_hz,
        },
    }
...
def build_frame_2ch(dt_us: int, dac_v: int, dac_i: int, flags: int) -> bytes:
    ...
    raw10 = struct.pack("<BI2HB", MAGIC0, dt_us, dac_v, dac_i, flags)
    return raw10 + bytes([checksum8(raw10), MAGIC1])
...
def main() -> None:
    ...
    parser.add_argument("--port", help="Porta serial da ESP32, ex.: /dev/ttyUSB0")
    ...
    parser.add_argument(
        "--receiver-config",
        help=(
            "Caminho opcional para salvar JSON de configuração da recepção "
            "(escala nominal/RMS e mapeamento). "
            "Se omitido, gera '<nome_cfg>_receiver_config.json' no diretório atual."
        ),
    )
    ...
    parser.add_argument(
        "--export-config-only",
        action="store_true",
        help="Gera apenas o JSON de configuração da recepção e encerra.",
    )
    ...
    full_samples, v_nom, i_nom, v_clip, i_clip = build_samples(cfg, bdat_path)
    ...
    if args.receiver_config:
        receiver_cfg_path = Path(args.receiver_config)
    else:
        receiver_cfg_path = Path.cwd() / f"{cfg_path.stem}_receiver_config.json"
    export_receiver_config_json(
        out_path=receiver_cfg_path,
        cfg_path=cfg_path,
        bdat_path=bdat_path,
        cfg=cfg,
        v_nom=v_nom,
    ...
    if args.export_config_only:
        print("ℹ️  Exportação de config concluída (modo --export-config-only).")
        return

    if not args.port:
        raise SystemExit("Informe --port para reprodução na ESP32, ou use --export-config-only.")
