...
def send_samples(port: str, baud: int, samples: List[SamplePacket], ready_timeout: float) -> None:
    ser = wait_esp32_ready(port, baud, timeout_s=ready_timeout)
    try:
        ...
        ser.write(build_legacy_frame(0, DAC_MID, DAC_MID, FLAG_IDLE_AFTER | FLAG_RESET_CLOCK))
        ser.flush()
