...
def save_generator_csv(path: Path, samples: List[SamplePacket]) -> None:
...
def send_idle_frame(ser: serial.Serial, reset_clock: bool = True) -> None:
...
def send_one_cycle(
    ...
    for idx, s in enumerate(samples, start=1):
        if ctrl.is_stop_requested():
            send_idle_frame(ser, reset_clock=True)
            return ctrl.get()
...
def run_interactive(ser: serial.Serial, samples: List[SamplePacket]) -> None:
    ctrl = CommandController()
    ...
    while True:
        ...
        if cmd == "single":
            final_cmd = send_one_cycle(ser, samples, ctrl)
            if final_cmd == "exit":
                send_idle_frame(ser, reset_clock=True)
                break
            if ctrl.get() == "single":
...
def main() -> None:
    ...
    if args.out:
        ...
        save_generator_csv(out_path, samples)
