#!/usr/bin/env python3
"""
Uploader/terminal for embedded_generator.ino.
...
def write_receiver_config(out_path: Path, cfg_path: Path, bdat_path: Path, status: dict[str, dict[str, str]]) -> None:
    ...
    payload = {
        "schema_version": 2,
        "generator_script": "run_embedded_generator.py",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Envia COMTRADE para a ESP32 geradora e encaminha comandos de reproducao."
    )
    ...
    parser.add_argument(
        "--receiver-config",
        help="Caminho para salvar o arquivo de configuracao. Default: 'config.json'",
    )
    parser.add_argument(
        "--upload-delay",
        type=float,
    ...
    try:
        ...
        receiver_config_path = Path(args.receiver_config) if args.receiver_config else Path.cwd() / "config.json"
