...
// ✅ novo: calcula pico nominal (por 5 ciclos) e limites de falta
...
static bool computeNominalPeaksFromFirstCycles(
  ...
  out_V_clip_peak = out_V_nom_peak * V_FAULT_LIMIT_MULT; // 3x
  out_I_clip_peak = out_I_nom_peak * I_FAULT_LIMIT_MULT; // 5x
  return true;
}

// ==============================
// ✅ Map ENG -> volts
// Agora o "full-scale" do DAC é o CLIP_PEAK (3x ou 5x do nominal),
// e só clipa acima disso.
//
// - Até CLIP_PEAK: linear
// - Acima: clip (subentende extrapolação do nominal)
// ==============================
static inline float mapEngToVolts_ClipPeakFS(float eng, float clipPeak) {
  ...
  // normaliza pelo clipPeak => ±clipPeak vira ±1 no DAC
...
// ==============================
// ✅ Novo: máquina de estados + loop do 1º ciclo
// ==============================
enum RunState : uint8_t {
  STATE_IDLE = 0,            // 1.65V fixo aguardando comando
  STATE_LOOP_FIRST_CYCLE,    // loop infinito do 1º ciclo
  STATE_PLAY_REST            // toca o restante do BDAT (a partir do 2º ciclo)
};

static RunState g_state = STATE_IDLE;
static bool     g_pendingFault = false; // recebeu 'f' enquanto em loop
...
static uint32_t g_dtLoopUs = 0;          // passo de tempo fixo no loop (1/fs)
static float*   g_cycleEngV = nullptr;   // [samplesPerCycle]
static float*   g_cycleEngI = nullptr;   // [samplesPerCycle]
static int64_t  g_lastTsFirstCycle = 0;  // ts da última amostra do 1º ciclo

// Execução
static uint32_t g_cycleIdx = 0;
static uint32_t g_logSample = 0;         // contador virtual de amostras gravadas
static uint32_t g_tAccumUs = 0;

static File g_csv;
static bool g_csvOpen = false;
static File g_bdat;
...
static bool openCsvAndWriteHeader() {
...
static inline void csvLog(uint32_t sampleNum, uint32_t t_us, uint32_t dt_us,
                          uint16_t dacV, uint16_t dacI,
                          uint16_t adcV_raw, uint16_t adcI_raw,
                          float adcV_V, float adcI_V,
                          int clipV, int clipI) {
  if (!g_csvOpen) return;
  g_csv.print(sampleNum); g_csv.print(',');
  g_csv.print(t_us);      g_csv.print(',');
  g_csv.print(dt_us);     g_csv.print(',');
  g_csv.print(dacV);      g_csv.print(',');
  g_csv.print(dacI);      g_csv.print(',');
  g_csv.print(adcV_raw);  g_csv.print(',');
  g_csv.print(adcI_raw);  g_csv.print(',');
  g_csv.print(adcV_V, 6); g_csv.print(',');
  g_csv.print(adcI_V, 6); g_csv.print(',');
  g_csv.print(clipV);     g_csv.print(',');
  g_csv.println(clipI);
}
...
static void runLoopFirstCycleStep() {
  uint32_t dt_us = (g_logSample == 0) ? 0u : g_dtLoopUs;
  ...
  csvLog(g_logSample, g_tAccumUs, dt_us, dacV, dacI, adcV_raw, adcI_raw, adcV_V, adcI_V, clipV, clipI);
  g_logSample++;
...
static void runPlayRestStep() {
  ...
  csvLog(sampleNum, g_tAccumUs, dt_us, dacV, dacI, adcV_raw, adcI_raw, adcV_V, adcI_V, clipV, clipI);
}
...
void setup() {
  ...
  // ✅ calcula nominal dinâmico (5 ciclos) e limites
  ...
  Serial.println("\nComandos:");
  Serial.println("  s = iniciar loop contínuo do 1º ciclo (e iniciar gravação CSV)");
  Serial.println("  f = (durante o loop) tocar o restante do COMTRADE após terminar o ciclo atual");
  Serial.println("  Q = parar tudo e voltar a 1.65V (encerra gravação se estiver gravando)\n");
}
  ...
  while (Serial.available()) {
    ...
    if (c == 's' || c == 'S') {
      if (g_state == STATE_IDLE) {
        if (!openCsvAndWriteHeader()) {
          Serial.println("❌ Não abriu output.csv");
          stopAllAndIdle();
        } else {
          Serial.println("▶️  Loop do 1º ciclo iniciado (gravando CSV)");
