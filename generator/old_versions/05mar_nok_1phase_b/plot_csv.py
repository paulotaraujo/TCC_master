...
def read_rows(path: Path) -> Tuple[List[float], Dict[str, List[float]]]:
    ...
    Suporta vários layouts (A/B, V/I, etc) via aliases.
    """
    ...
    with path.open("r", newline="", errors="ignore") as f:
        ...
        if not has_header:
            ...
            # 12: ... + adcA_V,adcB_V
            if ncols == 9:
                fieldnames = ["sample", "ts_comtrade", "dt_us", "engA", "engB",
                              "adcA_V", "adcB_V", "dacA_code", "dacB_code"]
            elif ncols == 10:
                fieldnames = ["sample", "ts_comtrade", "dt_us", "t_us", "engA", "engB",
                              "voutA", "voutB", "dacA_code", "dacB_code"]
            elif ncols == 12:
                fieldnames = ["sample", "ts_comtrade", "dt_us", "t_us", "engA", "engB",
                              "voutA", "voutB", "dacA_code", "dacB_code", "adcA_V", "adcB_V"]
            else:
            ...
            # ✅ Deixa o DictReader ler o header sozinho (evita chaves quebradas)
            f.seek(0)
        ...
        for r in rows:
            ...
            if (r.get("sample") is None or str(r.get("sample")).strip() == "") and \
               (r.get("ts_comtrade") is None or str(r.get("ts_comtrade")).strip() == "") and \
               (r.get("t_us") is None or str(r.get("t_us")).strip() == ""):
                continue
            cleaned.append(r)
        ...
        use_t_us = "t_us" in rows[0] and rows[0].get("t_us") is not None
        ...
        # =========
        # Colunas possíveis (tolerante + aliases)
        # =========
        for r in rows:
            # engenharia (layout antigo)
            if "engA" in r:   push("engA", to_float(r.get("engA")))
            if "engB" in r:   push("engB", to_float(r.get("engB")))

            # engenharia (mono layout novo, se você salvar engV/engI no futuro)
            if "engV" in r:   push("engA", to_float(r.get("engV")))  # alias -> engA
            if "engI" in r:   push("engB", to_float(r.get("engI")))  # alias -> engB

            # vout (layout antigo)
            if "voutA" in r:  push("voutA", to_float(r.get("voutA")))
            if "voutB" in r:  push("voutB", to_float(r.get("voutB")))

            # vout (mono layout novo, se você salvar vOutV/vOutI)
            if "vOutV" in r:  push("voutA", to_float(r.get("vOutV")))  # alias
            if "vOutI" in r:  push("voutB", to_float(r.get("vOutI")))  # alias

            # ADC (layout antigo)
            if "adcA_V" in r: push("adcA_V", to_float(r.get("adcA_V")))
            if "adcB_V" in r: push("adcB_V", to_float(r.get("adcB_V")))

            # ✅ ADC (mono layout novo: adcV_V/adcI_V)
            if "adcV_V" in r: push("adcA_V", to_float(r.get("adcV_V")))  # alias -> adcA_V
            if "adcI_V" in r: push("adcB_V", to_float(r.get("adcI_V")))  # alias -> adcB_V

            # DAC codes (antigo)
            if "dacA_code" in r: push("dacA_code", to_float(r.get("dacA_code")))
            if "dacB_code" in r: push("dacB_code", to_float(r.get("dacB_code")))

            # ✅ DAC codes (mono layout novo: dacV_code/dacI_code)
            if "dacV_code" in r: push("dacA_code", to_float(r.get("dacV_code")))  # alias
            if "dacI_code" in r: push("dacB_code", to_float(r.get("dacI_code")))  # alias
...
def plot_two_figures(t: List[float], data: Dict[str, List[float]], mode: str) -> None:
    ...
    if mode == "vout":
        ...
        yA = data["dacA_code"]
        yB = data["dacB_code"]
        ylabA = "DAC (códigos) - canal A"
        ylabB = "DAC (códigos) - canal B"
        titleA = "Códigos enviados ao DAC - Canal A"
        titleB = "Códigos enviados ao DAC - Canal B"
...
def main() -> None:
    ...
    ap.add_argument("--mode", choices=["auto", "vout", "eng", "adc", "dac"], default="auto",
                    help="auto=prioriza vout -> eng -> adc -> dac")
    args = ap.parse_args()
    ...
    if args.mode == "auto":
        if "voutA" in data and "voutB" in data:
            ...
            mode = "adc"
        elif "dacA_code" in data and "dacB_code" in data:
            mode = "dac"
        else:
            raise SystemExit(
                "Não encontrei pares de colunas válidos "
                "(voutA/voutB, engA/engB, adcA_V/adcB_V ou dacA_code/dacB_code)."
            )
    else:
        mode = args.mode
    ...
    if mode == "vout" and not ("voutA" in data and "voutB" in data):
        raise SystemExit("CSV não contém voutA/voutB (nem aliases).")
    if mode == "eng" and not ("engA" in data and "engB" in data):
        raise SystemExit("CSV não contém engA/engB (nem aliases).")
    if mode == "adc" and not ("adcA_V" in data and "adcB_V" in data):
        raise SystemExit("CSV não contém adcA_V/adcB_V (nem aliases).")
    if mode == "dac" and not ("dacA_code" in data and "dacB_code" in data):
        raise SystemExit("CSV não contém dacA_code/dacB_code (nem aliases).")
