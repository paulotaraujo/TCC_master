...
// Picos calculados a partir dos primeiros ciclos (pré-cálculo em setup)
...
void setup() {
  ...
  // ✅ calcula nominal dinâmico (5 ciclos) e limites (3x/5x)
  g_vNomPeak = 0.0f; g_iNomPeak = 0.0f;
  g_vClipPeak = 0.0f; g_iClipPeak = 0.0f;
  ...
  Serial.printf("Limites: V_clip=%.6f (%.1fx) | I_clip=%.6f (%.1fx)\n",
    ...
    // Saída em repouso: sem sinal (DC em V_MID). Se você realmente quiser 0.0V físico,
  // troque V_MID por 0.0f (mas lembre que não há tensão negativa no DAC).
  uint16_t idle = voltsToDac12(V_MID);
  dac.setChannelValue(MCP4728_CHANNEL_A, idle, MCP4728_VREF_VDD, MCP4728_GAIN_1X, MCP4728_PD_NORMAL, false);
  dac.setChannelValue(MCP4728_CHANNEL_B, idle, MCP4728_VREF_VDD, MCP4728_GAIN_1X, MCP4728_PD_NORMAL, true);

  Serial.println();
  Serial.println("🟡 Pronto. Envie 's' (start) no Serial Monitor para reproduzir o COMTRADE.");
  Serial.println("    Enquanto isso, a saída fica em repouso (V_MID).");
}
...
// Reproduz uma vez e salva output.csv
static void playComtradeOnce() {
  // abre CSV (sobrescreve)
  ...
  // garante saída inicial em repouso
  setIdleOutput();
  ...
  for (uint32_t i = 0; i < g_cfg.totalRecords; i++) {
    ...
    // permite cancelar com 'q' durante a reprodução
    if (Serial.available()) {
      int c = Serial.read();
      if (c == 'q' || c == 'Q') {
        Serial.println("🛑 Cancelado pelo usuário (q).");
        break;
      }
    }
  }
  ...
  Serial.println("🟡 Aguardando novo comando 's'...");
}

void loop() {
  // Mantém saída em repouso até receber comando
  setIdleOutput();

  if (!g_promptPrinted) {
    Serial.println("Digite 's' para iniciar a reprodução (ou 'q' para cancelar durante).");
  ...
  if (Serial.available()) {
    int c = Serial.read();
    if (c == 's' || c == 'S') {
      Serial.println("▶️ Iniciando reprodução...");
