...
// ==============================
static const int   NOM_CYCLES = 5;              // ✅ 5 ciclos para estimar nominal
static const float V_FAULT_LIMIT_MULT = 3.0f;   // ✅ tensão clipa acima de 3x nominal
static const float I_FAULT_LIMIT_MULT = 5.0f;   // ✅ corrente clipa acima de 5x nominal
...
void setup() {
  ...
  for (uint32_t i = 0; i < g_cfg.totalRecords; i++) {
    ...
    // ✅ mapeia para DAC:
    // agora: full-scale do DAC = CLIP_PEAK
    // => corrente só "chapa" acima de 5x nominal
    float vOutV = mapEngToVolts_ClipPeakFS(engV, vClipPeak); // 3x
    float vOutI = mapEngToVolts_ClipPeakFS(engI, iClipPeak); // 5x
