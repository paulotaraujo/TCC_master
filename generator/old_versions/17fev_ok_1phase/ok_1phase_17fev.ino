...
// Ajustes
// ==============================
// Serial Plotter: 1 = 2000 linhas/s | 2 = 1000 | 5 = 400 | 10 = 200
...
static bool replayToRamAndPlot(ComtradeCfg &cfg) {
  ...
  for (uint32_t i = 0; i < cfg.totalRecords; i++) {
    ...
    float adcA_V = adcRawToVolts12b(adcA_raw);
    float adcB_V = adcRawToVolts12b(adcB_raw);

    // Serial Plotter: só números (sem texto)
    if ((i % PLOT_DECIM) == 0) {
      Serial.print(adcA_V, 6);
      Serial.print(" ");
      Serial.println(adcB_V, 6);
    }
...
void setup() {
  ...
  // replay: durante essa fase, o Serial Plotter recebe só (adcA_V adcB_V)
