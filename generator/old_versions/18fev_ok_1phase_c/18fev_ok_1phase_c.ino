...
#define MAX_PREFAULT_SAMPLES 5000              // Buffer seguro (ex: 960Hz * 5 = 4800)
...
void setup() {
  ...
  // Calcular samples dos primeiros 5 ciclos
  ...
  // ==============================
  // FASE 1: Carregar buffer com 5 ciclos
  // ==============================
  Serial.println("Carregando buffer com pré-falta (primeiros 5 ciclos)...");
  ...
  // ==============================
  // FASE 2: Loop de pré-falta (reproduz buffer)
  // ==============================
  preFaultBuffer.reset();
  uint32_t sampleCount = 0;
  bool waitingForTrigger = true;
  t_accum = preFaultBuffer.getLastTimestamp(); // Manter continuidade temporal
  ...
  while (waitingForTrigger) {
    ...
    csv.print("0"); csv.print(","); // clipV (não aplicável no pré-falta)
    csv.print("0"); csv.print(","); // clipI
    csv.println("PRE"); // Modo pré-falta
    
    sampleCount++;
    
    // Verificar comando serial a cada SERIAL_CHECK_INTERVAL samples
    if (sampleCount % SERIAL_CHECK_INTERVAL == 0) {
      if (Serial.available()) {
        ...
        if (cmd == 'f' || cmd == 'F') {
          ...
          // Posicionar no início da falta (após os 5 ciclos)
          bdat.seek(preFaultSamples * g_cfg.recSize);
          
          // Registrar no CSV a transição
          csv.print(sampleCount); csv.print(",");
          csv.print(t_accum); csv.print(",");
          ...
          preFaultBuffer.reset();
          sampleCount = 0;
          t_accum = preFaultBuffer.getLastTimestamp();
          Serial.println("Reiniciando ciclo de pré-falta...");
        } else if (cmd == 's' || cmd == 'S') {
          Serial.printf("Status: %d samples reproduzidos, tempo=%.3fs\n", 
                       sampleCount, t_accum / 1000000.0f);
        }
      }
    }
  ...
  for (uint32_t i = preFaultSamples; i < g_cfg.totalRecords; i++) {
    ...
    if (i > preFaultSamples) {
      ...
      dt_us = preFaultBuffer.dt_us[preFaultBuffer.size - 1];
      if (dt_us > 0) busyWaitMicros(dt_us);
    }
    prevTs = ts;
    t_accum += dt_us;
    ...
    // Verificar comandos durante a falta (opcional)
