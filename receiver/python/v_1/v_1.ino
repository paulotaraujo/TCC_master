...
// [8:9]  adc_34 (uint16 LE, GPIO34)
// [10:11] adc_35 (uint16 LE, GPIO35)
// [12] checksum = sum(frame[0..11]) & 0xFF

#include <Arduino.h>

static const int ADC_PIN_34 = 34;           // ADC1: 32,33,34,35,36,39
static const int ADC_PIN_35 = 35;           // ADC1: 32,33,34,35,36,39
...
void setup() {
  ...
  analogSetPinAttenuation(ADC_PIN_34, ADC_11db); // faixa ~0..3.3V
  analogSetPinAttenuation(ADC_PIN_35, ADC_11db); // faixa ~0..3.3V
}
...
void loop() {
  ...
  uint16_t adc_34 = readAdcAvg(ADC_PIN_34, N_AVG);
  uint16_t adc_35 = readAdcAvg(ADC_PIN_35, N_AVG);
  if (adc_34 > 4095) adc_34 = 4095;
  if (adc_35 > 4095) adc_35 = 4095;
  ...
  frame[8] = (uint8_t)(adc_34 & 0xFF);
  frame[9] = (uint8_t)((adc_34 >> 8) & 0xFF);
  frame[10] = (uint8_t)(adc_35 & 0xFF);
  frame[11] = (uint8_t)((adc_35 >> 8) & 0xFF);
  frame[12] = checksum8(frame, 12);
