...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    bloco/.style={draw, rounded corners, align=center, text width=2.0cm, minimum height=1.05cm, font=\footnotesize},
    seta/.style={-{Latex[length=2.5mm]}, thick},
    dupla/.style={{Latex[length=2.5mm]}-{Latex[length=2.5mm]}, thick}
  ]
    \node[bloco, text width=2.2cm] (sim) {Matlab/\\Simulink\\simulação e\\COMTRADE};
    \node[bloco, right=of sim] (pcg) {Interface Python\\de geração\\upload, comandos\\e terminal};
    \node[bloco, text width=2.3cm, right=of pcg] (gen) {ESP32 geradora\\processamento,\\buffer e controle\\do DAC};
    \node[bloco, right=of gen] (dac) {MCP4728\\conversão D/A\\de tensão e\\corrente};
    \node[bloco, below=of gen] (rx) {ESP32 receptora\\aquisição, fasores\\e proteções};
    \node[bloco, left=of rx] (pcr) {Interface Python\\de recepção\\ajustes, eventos\\e terminal};
    \node[bloco, right=of rx] (led) {Painel físico\\estado, bloqueio,\\permissão e trip};

    \draw[seta] (sim) -- node[above=3pt, font=\tiny]{\texttt{.cfg/.bdat}} (pcg);
    \draw[dupla] (pcg) -- node[above=3pt, font=\tiny]{USB/serial} (gen);
    \draw[seta] (gen) -- node[above=3pt, font=\tiny]{I\textsuperscript{2}C} (dac);
    \draw[seta] (dac) -- node[right, font=\tiny, align=center]{V e I\\analógicas} (rx);
    \draw[dupla] (pcr) -- node[above=3pt, font=\tiny]{USB/serial} (rx);
    \draw[seta] (rx) -- (led);
    \draw[seta, dashed] (pcg.south) --
      node[right, fill=white, inner sep=2pt, font=\scriptsize]{\texttt{config.json}}
      (pcr.north);
  \end{tikzpicture}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.4cm}p{9.4cm}}
    ...
    \midrule
    Computador & Executar Matlab/Simulink e scripts Python, armazenar os registros, transferir arquivos, enviar ajustes e exibir eventos. \\
    ...
    LEDs/saídas digitais & Indicar estado do sistema, trip, bloqueio, permissão e estados das zonas de distância. \\
...
Os ensaios e o desenvolvimento foram realizados em um notebook Avell A70 HYB com arquitetura x86--64, processador Intel Core i7-12700H --- 14 núcleos, 20 \textit{threads} e frequência máxima de \SI{4,7}{\giga\hertz} \cite{intel_i7_12700h} --- e \SI{16}{\gibi\byte} de memória RAM. O sistema operacional utilizado foi o Ubuntu 24.04.4 LTS (Noble Numbat), pertencente à série Ubuntu 24.04 LTS. Essa configuração corresponde especificamente ao equipamento empregado no trabalho, uma vez que o modelo A70 HYB pode ser comercializado com diferentes combinações de memória e armazenamento.

O computador executa o MATLAB/Simulink R2025b, versão utilizada na modelagem do sistema elétrico e na geração dos vetores de tempo, tensão e corrente, além dos scripts Python, da configuração serial e do registro das mensagens. Entretanto, a reprodução temporizada e a decisão das proteções permanecem nas unidades embarcadas. Assim, a capacidade computacional do notebook contribui para a preparação e a supervisão dos ensaios, mas não integra diretamente o caminho de tempo crítico.
...
O DAC possui resolução de 12 bits, de modo que sua faixa digital contém 4096 códigos. A referência usada no firmware é \SI{3,3}{\volt}; assim, o código central 2048 produz aproximadamente \SI{1,65}{\volt}. Esse nível é utilizado tanto como ponto de repouso quanto como deslocamento de nível das formas de onda alternadas.
...
O firmware receptor configura os GPIOs 35 e 34 como entradas analógicas de tensão e corrente, respectivamente. A resolução do ADC é de 12 bits e a atenuação de \SI{11}{\decibel} é habilitada para compatibilizar a faixa de entrada com a saída do DAC. A correspondência de canais adotada no firmware é, portanto, GPIO35 para tensão e GPIO34 para corrente.

A ESP32 receptora executa todas as funções de proteção implementadas na plataforma: sobrecorrente instantânea (ANSI 50), sobrecorrente temporizada (ANSI 51), subtensão (ANSI 27), sobretensão (ANSI 59), distância (ANSI 21) e supervisão direcional (ANSI 67). Essas funções podem ser habilitadas ou desabilitadas individualmente pelo script de configuração e são avaliadas a partir dos mesmos fasores fundamentais de tensão e corrente.

As quatro saídas digitais GPIO14, GPIO27, GPIO26 e GPIO25 não representam a totalidade das proteções existentes. As duas primeiras indicam os estados gerais da supervisão 67: bloqueio e habilitação, respectivamente. As duas últimas detalham a atuação das zonas 1 e 2 da função 21. De acordo com a filosofia definida para esta plataforma, a lógica 67 pode supervisionar todas as funções atuantes implementadas: 21, 27, 50, 51 e 59. Assim, quando a função direcional e qualquer uma dessas proteções são configuradas conjuntamente, a condição direcional autoriza ou bloqueia sua atuação. Quando a função 67 não está habilitada, cada proteção opera exclusivamente segundo seus próprios critérios de pickup e temporização.

As funções 27, 59, 50 e 51 são identificadas nominalmente pelas mensagens de eventos na interface serial e também contribuem para o sinal geral de trip indicado pelo GPIO12. Dessa forma, a indicação geral do painel é complementada pelo detalhamento fornecido no terminal.

O GPIO13 indica que o sistema está pronto, enquanto o GPIO12 pisca quando existe qualquer trip retido, independentemente de sua origem. As saídas foram implementadas como ativas em nível alto; essa escolha deve ser revista caso seja empregado um módulo de relés com entradas ativas em nível baixo.
...
\begin{table}[H]
  ...
  \begin{tabular}{cll}
    ...
    26 & Saída digital & Trip da zona 1 da função 21 \\
    25 & Saída digital & Pickup ou trip da zona 2 da função 21 \\
...
A interligação física adotada segue o mapa compilado no firmware receptor: tensão no GPIO35 e corrente no GPIO34. O JSON fornece as escalas e os valores de referência associados a esses canais, preservando a correspondência entre as grandezas reproduzidas e adquiridas. Como a reconstrução final depende do ganho efetivo do DAC, da entrada ADC e da montagem física, o script receptor também admite fatores multiplicativos de correção de escala. Esses fatores são aplicados sobre as escalas do JSON antes do envio à ESP32 receptora e permitem ajustar a leitura de pré-falta aos valores nominais do registro.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{4.0cm}p{8.8cm}}
    ...
    \midrule
    \path{comtrade_generator.m} & Extrair os vetores do Matlab, gerar gráficos e produzir um COMTRADE ASCII trifásico inicial. \\
    ...
    \path{embedded_receiver.ino} & Adquirir os canais, remover offset, estimar fasores, avaliar proteções e controlar as saídas. \\
    \bottomrule
  \end{tabular}
  \legend{Fonte: Autoria própria.}
\end{table}
...
O script Matlab lê \texttt{Vin.Time}, as três colunas de \texttt{Vin.Data} e as três colunas de \texttt{Iin.Data}. A taxa de amostragem é estimada por

\begin{equation}
  f_s=\left[\frac{1}{N-1}\sum_{k=1}^{N-1}\left(t[k]-t[k-1]\right)\right]^{-1}.
\end{equation}

Além de gerar os gráficos trifásicos, o script escreve um arquivo \texttt{.cfg} com seis canais analógicos e um arquivo \texttt{.dat} ASCII. Os coeficientes $a=1$ e $b=0$ escritos nesse arquivo indicam que os valores já se encontram em unidades de engenharia.

Entretanto, o processador COMTRADE implementado em \path{embedded_generator.ino} aceita apenas o formato \texttt{BINARY}. Os arquivos efetivamente usados nos testes são pares \texttt{export.cfg}/\texttt{export.bdat}, com dois canais analógicos ordenados como tensão e corrente. Portanto, entre a saída original do Matlab e o upload existe uma etapa de exportação ou conversão para binário e de seleção da fase. Essa etapa deve ser mantida documentada, pois o firmware interpreta obrigatoriamente o primeiro canal como tensão e o segundo como corrente, independentemente dos nomes declarados no \texttt{.cfg}.

O parser embarcado lê no máximo 160 linhas do \texttt{.cfg}, extrai o número de canais, os coeficientes $a$ e $b$ dos dois primeiros canais, a frequência, a primeira taxa de amostragem, o formato e o multiplicador de tempo. O valor bruto de cada canal é convertido por
...
O leiaute do arquivo binário é inferido a partir do tamanho total. Para marcas de tempo de 32 bits, cada registro ocupa

\begin{equation}
  L_{32}=4+4+2N_A+2\left\lceil\frac{N_D}{16}\right\rceil \quad \text{bytes},
\end{equation}

em que $N_A$ e $N_D$ são as quantidades de canais analógicos e digitais. Para marcas de tempo de 64 bits, o segundo termo passa de 4 para 8 bytes. O firmware verifica qual tamanho divide exatamente o arquivo e determina o número total de registros. Arquivos ASCII, vazios ou cujo tamanho não seja compatível são rejeitados antes da criação dos quadros.

\section{Interface Python da unidade geradora}

\subsection{Abertura da porta e transferência dos arquivos}

O script \path{run_embedded_generator.py} utiliza a biblioteca \texttt{pyserial} e opera, por padrão, à taxa serial de 921600 baud. Após abrir a porta, desativa as linhas DTR e RTS para reduzir reinicializações involuntárias, aguarda a inicialização da placa e procura as mensagens \texttt{READY\_UPLOAD} ou \texttt{READY\_PLAYBACK}.

Cada arquivo é enviado em blocos de 256 bytes. Antes dos dados, o computador transmite \texttt{UPLOAD <tipo> <tamanho>}. A ESP32 responde \texttt{READY\_RECEIVE}, grava os bytes no LittleFS e confirma cumulativamente cada bloco por meio de \texttt{RX <bytes>}. Ao final, devolve \texttt{OK CFG} ou \texttt{OK BDAT}. O computador só envia o bloco seguinte depois de receber a confirmação esperada, evitando que o buffer serial seja sobrecarregado.

\begin{table}[H]
  \centering
  \small
  \caption{Sequência do protocolo de upload.}
...
O firmware impõe um tempo máximo de \SI{15}{\second} sem progresso durante o upload. Falhas de sintaxe, tipo, tamanho, escrita ou tempo esgotado produzem mensagens iniciadas por \texttt{ERR}. Esse mecanismo facilita o diagnóstico sem exigir um protocolo binário adicional de controle.

\subsection{Geração do arquivo JSON}

Após o comando \texttt{PROCESS}, o script coleta as linhas \texttt{STATUS CFG} e \texttt{STATUS SCALE}. A partir delas, cria o \texttt{config.json}. Os dados não são recalculados de forma independente no computador; eles refletem as referências efetivamente obtidas pela ESP32 geradora. Isso reduz o risco de usar fórmulas distintas nos dois lados da plataforma.
...
Depois da criação do JSON, o script inicia uma thread dedicada à leitura das mensagens da ESP32 e mantém o terminal principal disponível para comandos. A Tabela \ref{tab:comandos-gerador} resume os comandos implementados.
...
Os quadros são encaminhados ao DAC por um buffer circular de 4096 posições. Os índices de escrita e leitura, assim como a quantidade ocupada, são protegidos por um mutex. Outro mutex protege o comando pendente. Essa sincronização impede que as tarefas alterem simultaneamente as mesmas estruturas.

O firmware cria três tarefas, apresentadas na Tabela \ref{tab:tarefas-geradora}. A separação entre controle, comunicação e atualização do DAC evita que uma leitura serial demorada interrompa diretamente o instante de saída de uma amostra.
...
A tarefa do DAC acumula os intervalos relativos em uma marca absoluta baseada em \texttt{micros()}. Quando o instante programado é alcançado, os canais A e B são atualizados por uma única operação rápida do MCP4728. Se o buffer esvaziar antes do esperado, um contador de \textit{underflow} é incrementado e a reprodução é interrompida até que existam novos quadros. Ao término de uma reprodução unitária, um quadro especial recoloca todos os canais no código 2048.

No modo de pré-falta, a tarefa de controle mantém ciclos no buffer. O comando \texttt{f} acrescenta o registro completo a partir do segundo quadro, sem retornar antes ao nível médio. Esse encadeamento reduz a descontinuidade entre a pré-falta repetida e o evento.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    node distance=1.55cm and 1.35cm,
    estado/.style={draw, rounded corners, align=center, minimum width=2.7cm, minimum height=0.9cm, font=\small},
    seta/.style={-{Latex[length=2.4mm]}, thick}
  ]
    \node[estado] (idle) {Repouso\\código 2048};
    \node[estado, right=of idle] (pref) {Pré-falta\\contínua};
    \node[estado, right=of pref] (full) {COMTRADE\\completo};
    \node[estado, below=of pref] (test) {Senoide\\de teste};
    \draw[seta] (idle) -- node[above, font=\scriptsize]{\texttt{s}} (pref);
    \draw[seta] (pref) -- node[above, font=\scriptsize]{\texttt{f}} (full);
    \draw[seta] (idle) to[bend left=25] node[above, font=\scriptsize]{\texttt{p}} (full);
    \draw[seta] (idle.south) to[out=-90,in=180]
      node[below left, font=\scriptsize]{\texttt{t}} (test.west);
    \node[below=0.45cm of test, align=center, font=\scriptsize] (retorno)
      {Retorno ao repouso: \texttt{q/x} ou término da reprodução};
  \end{tikzpicture}
  \legend{Fonte: Autoria própria.}
\end{figure}
...
Quando a opção \texttt{normalize\_to\_comtrade} está ativa, o firmware coleta dez ciclos válidos de pré-falta. Somente ciclos cujas magnitudes superem o limite mínimo configurado são aceitos. A mediana das magnitudes é comparada aos valores nominais do JSON, originando os ganhos
...
Quando $I_1\geq I_{50}$, a função 50 atende ao seu critério instantâneo. Quando $I_1\geq I_{51}$, a função 51 entra em pickup e acumula o tempo dos ciclos até o atraso ajustado. O retorno ocorre abaixo de $0{,}95I_{51}$ enquanto não houver trip, introduzindo uma histerese de 5\%. A curva implementada é de tempo definido; não há, nesta versão, curva inversa IEC ou IEEE. Se a função 67 estiver habilitada, o trip da 50 e o avanço do temporizador da 51 também exigem permissão direcional; sem a 67, ambas operam apenas pelos critérios de corrente e tempo.

\subsection{Subtensão e sobretensão -- ANSI 27/59}

Os níveis de pickup são percentuais da tensão nominal eficaz:

\begin{equation}
  V_{27}=V_N\frac{p_{27}}{100}, \qquad
  V_{59}=V_N\frac{p_{59}}{100}.
\end{equation}

O pickup da função 27 é reconhecido imediatamente quando $V_1\leq V_{27}$, e o da função 59, quando $V_1\geq V_{59}$. Cada função possui atraso independente, definido em segundos; um ajuste igual a zero produz atuação sem atraso intencional, enquanto um valor positivo estabelece atuação temporizada. A função 27 retorna quando $V_1>1{,}03V_{27}$, e a função 59, quando $V_1<0{,}97V_{59}$. Portanto, ambas possuem histerese de 3\% no sentido de retorno. Quando a função 67 também está habilitada, o pickup continua sendo registrado, mas a temporização e o trip dependem da permissão direcional.
...
\begin{equation}
  ...
  \left(\neg E_{67}\lor P_{67}\right),
  \label{eq:permissivo-67}
\end{equation}

em que $\operatorname{op}_F$ representa o atendimento dos critérios próprios da função, $E_{67}$ indica que a supervisão direcional está habilitada e $P_{67}$ representa a permissão direcional. Dessa forma, a proteção opera normalmente quando a função 67 está desabilitada; quando a supervisão está ativa, a ausência de permissão bloqueia a atuação, mesmo que o pickup da função supervisionada tenha ocorrido.

A Figura \ref{fig:hierarquia-protecoes} sintetiza a hierarquia lógica implementada. Cada função é habilitada e ajustada de forma independente, formando sua própria condição de operação antes da supervisão direcional. A saída $G_{67}$ é aplicada igualmente às quatro ramificações; portanto, as portas AND não estabelecem prioridade entre as proteções, mas condicionam individualmente cada trip à filosofia direcional selecionada. A porta OR representa apenas a agregação dos trips já autorizados.

\begin{figure}[H]
  \centering
  \caption{Hierarquia lógica de execução e supervisão das funções de proteção.}
  \label{fig:hierarquia-protecoes}
  \input{figuras/hierarquia_protecoes.tex}
  \legend{Fonte: Autoria própria, com base na lógica implementada no firmware receptor.}
\end{figure}

Na Figura \ref{fig:hierarquia-protecoes}, $E_F$ representa a habilitação individual da função $F$, $P_F$ representa seu pickup e $T_F$ indica o atendimento da temporização correspondente. Para as funções 27 e 59, $T_F$ é imediatamente verdadeiro quando o atraso é ajustado em zero. Para a função 21, $P_{Z1}$ corresponde à zona 1 instantânea e $P_{Z2}\land T_{Z2}$ à zona 2 temporizada. O termo $P_{\Sigma}$ indica a existência de ao menos um pickup supervisionado, enquanto $P_{67}$ representa a concordância entre o sentido medido e o sentido configurado.

O critério direcional disponível no firmware estima a potência ativa por

\begin{equation}
  P_{67}=|V_1||I_1|\cos(\angle V_1-\angle I_1).
\end{equation}

Enquanto nenhuma das funções supervisionadas está em pickup, valores de potência cujo módulo supera o limiar configurado atualizam a referência de pré-falta. Potência de pré-falta positiva representa sentido direto e potência negativa, sentido reverso. O estado direcional é calculado uma única vez a cada ciclo válido e compartilhado pelos blocos 21, 27, 50, 51 e 59.

Para a função 21, a falta de permissão bloqueia tanto o trip instantâneo da zona 1 quanto a temporização da zona 2. Na 50, bloqueia o trip instantâneo; na 51, impede o avanço do temporizador. Para 27 e 59, o pickup permanece observável, porém o temporizador é mantido em zero e nenhum trip é emitido enquanto a direção não for válida. Quando a permissão passa a existir e a condição de pickup persiste, a temporização começa a partir de zero; com atraso configurado em zero, a atuação ocorre no primeiro ciclo válido permitido. Essa regra exige simultaneamente o critério próprio da proteção, a condição temporal correspondente e, quando solicitada, a condição direcional da função 67.
...
Quando habilitada, a emissão de eventos informa transições de pickup, temporização, retorno, bloqueio e trip. Mensagens de temporização são limitadas por um intervalo mínimo, reduzindo a ocupação da serial. Ao surgir qualquer trip, uma mensagem geral \texttt{BREAKER TRIP} reúne as fontes ativas.

Os trips permanecem retidos até o comando \texttt{resettrip}. Esse comportamento reproduz, de forma simplificada, a necessidade de reconhecimento de uma atuação. A saída de zona 2 permanece ativa tanto durante a temporização quanto depois do trip; por isso, sua indicação visual representa pickup/temporização ou atuação, conforme o instante observado.
...
\begin{table}[H]
  \centering
  \small
  \caption{Comandos principais do firmware receptor.}
  ...
  \begin{tabular}{ll}
    ...
    \texttt{status} & Exibir fasores, frequência, impedância, qualidade e trips \\
    \texttt{resettrip} & Limpar temporizadores, trips e saídas retidas \\
...
O comando \texttt{status} é especialmente útil para a implementação e a depuração. Ele apresenta quantidade de amostras e ciclos, $V_1$, $I_1$, frequência, validade da DFT, ganhos de normalização, impedância, resistência, reatância, estados das zonas, temporizador da zona 2, permissão direcional, trips e indicadores de qualidade.

\section{Montagem e execução integrada}

A montagem completa e a bancada foram apresentadas nas Figuras \ref{fig:sistema-completo} e \ref{fig:bancada-testes}, no capítulo de Metodologia. Na execução integrada, as duas placas são conectadas ao computador por portas seriais distintas. Depois do upload e do processamento do COMTRADE, a receptora é configurada com o JSON recém-gerado. A geradora deve permanecer inicialmente em pré-falta contínua; nessa condição, verifica-se se $V_1$ e $I_1$ medidos pela receptora coincidem com os valores nominais do JSON. Quando houver desvio sistemático de ganho, são aplicadas as correções de escala no comando da receptora. Somente após essa verificação são liberadas as proteções e, posteriormente, o evento completo.

Durante esse processo, qualquer função habilitada pode produzir pickup, temporização, retorno ou trip. Os eventos das funções 27, 59, 50, 51 e 21 são discriminados na comunicação serial. O painel físico fornece uma indicação geral para qualquer trip, duas indicações comuns de bloqueio e habilitação da supervisão 67 sobre 21, 27, 50, 51 e 59 e duas indicações específicas para as zonas da função 21.

\section{Tratamento de falhas e mecanismos de diagnóstico}

A implementação contém verificações em diferentes pontos do fluxo. Na geradora, são detectadas ausência do MCP4728, falha do LittleFS, falha de alocação, formato COMTRADE incompatível, leiaute binário inválido, timeout de upload, falta de quadros e esvaziamento do buffer. Na receptora, são sinalizados saturação, amostras discrepantes, ciclo incompleto, frequência inválida e comandos desconhecidos.

As mensagens iniciadas por \texttt{READY}, \texttt{STATUS}, \texttt{ACK}, \texttt{OK}, \texttt{WARN} e \texttt{ERR} formam uma interface textual de diagnóstico. Embora simples, essa padronização permite distinguir prontidão, progresso, confirmação e falha. Os scripts Python interrompem a sequência quando recebem \texttt{ERR} ou quando uma confirmação não chega dentro do prazo.

Como evolução do protocolo, trabalhos futuros poderão acrescentar CRC ao upload e armazenar no JSON um identificador do \texttt{.cfg} e do \texttt{.bdat}, ampliando a rastreabilidade dos arquivos utilizados em cada ensaio.
...
O \texttt{config.json} mantém a coerência de escala entre as duas unidades, enquanto os protocolos seriais garantem que arquivos e parâmetros sejam confirmados antes da execução. A combinação de buffer circular, tarefas FreeRTOS, controle de qualidade das amostras, retenção de trips e saídas físicas permite que o protótipo represente, de maneira observável, o caminho entre uma oscilografia simulada e a decisão de proteção. As recomendações apresentadas estabelecem possibilidades de continuidade e expansão da plataforma em trabalhos futuros.
