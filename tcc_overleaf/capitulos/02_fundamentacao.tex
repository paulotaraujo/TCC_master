...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    bloco/.style={draw, rounded corners, align=center, minimum width=2.45cm, minimum height=1.05cm},
    seta/.style={-{Latex[length=2.5mm]}, thick}
  ]
    \node[bloco] (geracao) {Geração};
    \node[bloco, right=of geracao] (trafo1) {Transformador\\elevador};
    \node[bloco, right=of trafo1] (transmissao) {Transmissão\\AT/EAT};
    \node[bloco, right=of transmissao] (sub) {Subestação};
    \node[bloco, below=of sub] (dist) {Distribuição};
    \node[bloco, left=of dist] (carga) {Cargas};

    \draw[seta] (geracao) -- node[above]{\small $V$, $I$, $P$, $Q$} (trafo1);
    \draw[seta] (trafo1) -- (transmissao);
    \draw[seta] (transmissao) -- node[above]{\small linha $Z=R+jX$} (sub);
    \draw[seta] (sub) -- (dist);
    \draw[seta] (dist) -- node[below]{\small consumo} (carga);
  \end{tikzpicture}
  \caption{Diagrama básico de um sistema elétrico de potência}
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
    escala/.style={font=\small},
    linha/.style={thick},
    falha/.style={thick, red}
  ]
    \draw[linha] (0,1.2) -- (7,1.2);
    \draw[linha] (0,0.6) -- (7,0.6);
    \draw[linha] (0,0.0) -- (7,0.0);

    \node[escala,left] at (0,1.2) {Fase A};
    \node[escala,left] at (0,0.6) {Fase B};
    \node[escala,left] at (0,0.0) {Fase C};

    \draw[linha] (-0.6,0.6) circle (0.35);
    \node[escala] at (-0.6,0.6) {G};

    \draw[linha] (7.2,1.2) -- (7.2,0.0);
    \node[escala,right] at (7.35,0.6) {Carga};

    \draw[falha] (4.2,1.2) -- (4.2,0.0);
    \draw[falha] (4.0,0.95) -- (4.4,0.65);
    \draw[falha] (4.4,0.65) -- (4.0,0.35);
    \draw[falha] (4.0,0.35) -- (4.4,0.05);
    \node[escala,red,above] at (4.2,1.35) {curto trifásico};

    \draw[-{Latex[length=2.5mm]}, thick] (1.2,1.45) -- node[above]{\small $I_{cc}$} (3.7,1.45);
  \end{tikzpicture}
...
\noindent Assim, a condição de atuação por subtensão pode ser representada por:

\begin{equation}
  V_{pu} \leq V_{27,\text{pickup}}.
  \label{eq:subtensao}
\end{equation}

Em aplicações digitais, essa condição normalmente é associada a uma temporização para evitar atuações indevidas durante transitórios rápidos. No contexto deste trabalho, a função de subtensão pode ser utilizada para observar o afundamento de tensão causado pela falta trifásica e verificar se o algoritmo identifica corretamente a permanência da tensão abaixo do limite configurado.
...
Utilizando a mesma representação em por unidade da Equação \ref{eq:vpu}, a condição de atuação por sobretensão pode ser expressa por:

\begin{equation}
  V_{pu} \geq V_{59,\text{pickup}}.
  \label{eq:sobretensao}
\end{equation}

Em uma plataforma didática, essa função permite demonstrar como limites de tensão são definidos e como a alteração dos parâmetros de ajuste modifica a decisão do relé. Em estudos de falta trifásica, sua aplicação pode ser observada principalmente em situações de recuperação pós-falta, rejeição de carga ou cenários simulados com elevação de tensão.

\subsection{Relé de sobrecorrente}

O relé de sobrecorrente é uma das funções mais utilizadas em sistemas de proteção. Ele atua quando a corrente medida ultrapassa um valor de pickup previamente definido. De acordo com a aplicação, pode assumir forma instantânea, associada ao número ANSI 50, ou temporizada, associada ao número ANSI 51 \cite{blackburn2006protective,ieee_c37_2_2022}.
...
Para a função temporizada, define-se normalmente um múltiplo da corrente de pickup:
...
O COMTRADE, sigla para \textit{Common Format for Transient Data Exchange}, é um padrão utilizado para armazenar e intercambiar dados transitórios e registros oscilográficos de sistemas elétricos de potência. O padrão é definido pela IEEE C37.111 e também corresponde à IEC 60255-24 \cite{ieee_comtrade,iec_60255_24_2013}. Ele estabelece uma forma comum de representar dados provenientes de registradores digitais de perturbação, relés de proteção, simulações computacionais e sistemas de medição.
...
\noindent A partir desse coeficiente, obtêm-se a magnitude e o ângulo do fasor:

\begin{align}
  |X_1| &= \sqrt{\operatorname{Re}(X_1)^2 + \operatorname{Im}(X_1)^2}, \label{eq:magnitude-fasor}\\
  \theta_X &= \tan^{-1}\left(\frac{\operatorname{Im}(X_1)}{\operatorname{Re}(X_1)}\right). \label{eq:angulo-fasor}
\end{align}
