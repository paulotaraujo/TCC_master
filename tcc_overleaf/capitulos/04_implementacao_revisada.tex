...
\hl{A escolha do MCP4728 seguiu a mesma lógica adotada para a ESP32: disponibilidade, baixo custo, simplicidade de integração e compatibilidade com a proposta didática. O conversor oferece quatro saídas analógicas de 12 bits por interface I\textsuperscript{2}C, o que foi suficiente para a reprodução de tensão e corrente de uma fase, mantendo ainda dois canais disponíveis. Entretanto, essa escolha também limita a expansão direta para uma bancada trifásica completa. Para reproduzir simultaneamente três tensões e três correntes seriam necessários, no mínimo, seis canais analógicos de saída, além de uma cadeia de aquisição correspondente na unidade receptora. Em uma evolução do projeto, poderiam ser empregados DACs multicanais com maior taxa de atualização, conversores com interface SPI, módulos de aquisição e geração dedicados ou placas de aquisição de dados com sincronismo entre canais.}
