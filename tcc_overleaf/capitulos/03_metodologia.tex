...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[bloco, right=of comtrade] (pcgen) {\texttt{generate\_signal\_v6.py}\\upload e controle};
    \node[bloco, below=of pcgen] (espgen) {ESP32 geradora\\\texttt{v\_6.ino}};
    \node[bloco, left=of espgen] (dac) {DAC MCP4728\\sinais $V$ e $I$};
    \node[bloco, below=of dac] (esprx) {ESP32 receptora\\\texttt{v\_2.ino}};
...
Na versão atual do projeto, o script \texttt{generate\_signal\_v6.py} atua como interface de upload e controle da ESP32 geradora. Diferentemente das primeiras versões, nas quais o computador realizava maior parte do tratamento das amostras, a versão atual transfere o processamento principal para a ESP32. Assim, o computador envia os arquivos e a placa realiza localmente a interpretação, escala e preparação dos quadros de reprodução.
...
O envio dos arquivos ocorre por comunicação USB/serial. O script \texttt{generate\_signal\_v6.py} abre a porta serial da ESP32 geradora, aguarda a mensagem de prontidão da placa, envia o arquivo \texttt{.cfg} e o arquivo de dados em blocos e verifica as confirmações de recebimento.
...
O procedimento experimental segue uma sequência padronizada para garantir repetibilidade dos ensaios. Inicialmente, seleciona-se a oscilografia COMTRADE correspondente à falta a ser analisada. Em seguida, o script \texttt{generate\_signal\_v6.py} envia os arquivos à ESP32 geradora e solicita o processamento local. Após a confirmação de prontidão, a unidade geradora permanece disponível para executar os modos de teste, pré-falta ou reprodução completa.
